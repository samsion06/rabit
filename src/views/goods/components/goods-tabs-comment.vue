...
<script>
  import { findCommentInfoByGoods, findCommentList } from '@/api/good'
  ...
  const getCommentInfo = (goods) => {
    ...
    findCommentInfoByGoods(goods.value.id).then(data => {
      ...
      console.log("评价头部数据", commentInfo.value);
    })
    return commentInfo
  }
  //切换头部标签/排序
  const changeTagEffect = (reqParams, commentInfo) => {
    ...
    const changeTag = (index) => {
      ...
      //修改查询参数
      if (currTag.type === 'all') { //全部
        reqParams.hasPicture = false
        reqParams.tag = null
      } else if (currTag.type === 'img') { //带图
    ...
    return { currTagIndex, changeTag, changeSort }
  }
  ...
  export default {
    ...
    setup(props) {
      /**
       * @修改标签的时候重新拉数据 （修改查询参数字段）
       * @修改排序也重新拉数据
       */
      const goods = inject('goods') //获取父组件商品详情数据
      // 查询参数
      ...
      //切换头部标签/排序
      const { currTagIndex, changeTag, changeSort } = changeTagEffect(reqParams, commentInfo)

      // 初始化或者筛选条件改变后，获取列表数据。
      const commentList = ref([])
      watch(reqParams, async () => {
        const data = await findCommentList(goods.value.id, reqParams)
