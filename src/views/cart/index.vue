<template>
  <div class="cart">
    <table>
      <thead>
        <!--标题 vuex的数据是不能进行双向绑定的，不能直接修改， $store.getters['cart/isCheckAll']-->
        <tr>
          <th width="120">
      ...
      <tbody>
        <tr v-for="item in $store.getters['cart/validList']" :key="item.skuId">
          ...
          :modelvalue可不是双向绑定 => v-model:value才是双向绑定
          -->
          <td>
          ...
          <td class="tc">
            <!--修改数量-->
            <XtxNumbox @change="$event=>changeCount(item.skuId,$event)" v-model:modelValue="item.count" />
            {{item.count}}
          </td>
          <td class="tc">
...
<script>
  ...
  export default {
    ...
    components: {},
    setup() {
      //https://www.bilibili.com/video/av547086864/?p=414&spm_id_from=pageDriver
      const isSelect = ref(false);
      const store = useStore()
      // 单选
      const checkOne = (skuId, selected) => {
        console.log("selected", selected, skuId);
      ...
      // 批量删除
      const batchDeleteCart = (skuId) => {
        window.confirm('确认删除吗？') ? store.dispatch('cart/batchDeleteCart', skuId) : ''
      }
      ...
      const goCheckout = () => {
        ...
        // 2. 判断是否已经登录，未登录 弹窗提示
        // 3. 进行跳转 （需要做访问权限控制）
        if (store.getters['cart/selectedTotal'] === 0) alert('至少选中一件商品才能结算')
